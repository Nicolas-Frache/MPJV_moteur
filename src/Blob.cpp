...
void Blob::addNode(Ball* ball){
	...
	for (int i = 0; i < this->nodes.size(); i++) {
		ball->applyForce(new RessortForce(ball, this->nodes[i], nodes[i]->size + ball->size, .1, 100));
		this->nodes[i]->applyForce(new RessortForce(this->nodes[i], ball, nodes[i]->size + ball->size, .1, 100));
	}
